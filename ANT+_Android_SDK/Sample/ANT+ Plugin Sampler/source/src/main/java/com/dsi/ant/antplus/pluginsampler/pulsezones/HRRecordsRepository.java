...
public class HRRecordsRepository {
    ...
    public int getMaxHeartRate() {
        Cursor cursor = databaseRead.rawQuery("SELECT MAX(heart_rate) FROM records", null);
        int value = 0;
    ...
    public int getAverageHeartRate() {
        Cursor cursor = databaseRead.rawQuery("SELECT AVG(heart_rate) FROM records", null);
        int value = 0;
        if (cursor.moveToFirst()) {
